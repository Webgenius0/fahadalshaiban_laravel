...
class LoginTutorialController extends Controller
{
   public function index(Request $request)
   {
    if ($request->ajax()) {
        $data = Tutorial::where('section', SectionEnum::AllUSER->value)->where('page', PageEnum::LOGIN_TUTORIAL->value)->latest()->get();
        return DataTables::of($data)
            ...
            ->addColumn('action', function ($data) {
                return '<div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                            <a href="' . route('admin.login.edit', ['id' => $data->id]) . '" type="button" class="btn btn-primary fs-14 text-white edit-icn" title="Edit">
    ...
    return view('backend.layouts.tutorial.login.index');
   }

   //create
   public function create()
   {
       return view('backend.layouts.tutorial.login.create');
   }
   ...
   public function store(Request $request)
    {
        ...
        try {
            ...
            $validatedData['section'] = SectionEnum::AllUSER->value;
            $validatedData['page'] = PageEnum::LOGIN_TUTORIAL->value;
            $validatedData['slug'] = Str::slug($validatedData['title']);
            if ($request->hasFile('video')) {
                $validatedData['video'] = Helper::fileUpload($request->file('video'), 'tutorial/login', time() . '_' . getFileName($request->file('video')));
            }
          if(Tutorial::create($validatedData)){
            return redirect()->route('admin.login.index')->with('t-success', 'Created successfully');
          }
    ...
    public function edit(string $id)
    {
        ...
        return view("backend.layouts.tutorial.login.update", compact("homeAbout"));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            return redirect()->route('admin.login.index')->with('t-success', 'Updated successfully');
        } catch (\Throwable $th) {

...
namespace App\Http\Controllers\Web\Backendeb\Backend\CMS\Tutorial;
...
class ClientCampaignController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Tutorial::where('section', SectionEnum::USER->value)->where('page', PageEnum::Add_CAMPING->value)->latest()->get();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    return '<div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                                <a href="' . route('admin.client.tutorial.edit', ['id' => $data->id]) . '" type="button" class="btn btn-primary fs-14 text-white edit-icn" title="Edit">
        ...
        return view('backend.layouts.tutorial.client.newcampaign.index');
    }

    //create
    public function create()
    {
        return view('backend.layouts.tutorial.client.newcampaign.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('video')) {
                $validatedData['video'] = Helper::fileUpload($request->file('video'), 'tutorial/client', time() . '_' . getFileName($request->file('video')));
            }
          if(Tutorial::create($validatedData)){
            return redirect()->route('admin.client.tutorial.index')->with('t-success', 'Created successfully');
          }
         
        } catch (\Throwable $th) {
            Log::error('Error during tutorial creation: ' . $th->getMessage());
    ...
    public function edit(string $id)
    {
        ...
        return view("backend.layouts.tutorial.client.newcampaign.update", compact("homeAbout"));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            if ($request->hasFile('video')) {
                ...
                $validatedData['video'] = Helper::fileUpload($request->file('video'), 'tutorial', time() . '_' . getFileName($request->file('video')));
               
            }
            $homeAbout->update($validatedData);
            return redirect()->route('admin.client.tutorial.index')->with('t-success', 'Updated successfully');
        } catch (\Throwable $th) {
            Log::error('Error during tutorial update: ' . $th->getMessage());
            return redirect()->back()->with('t-error', $th->getMessage());
        }
    }

...
class HomeTutorialController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $data = Tutorial::where('section', SectionEnum::OWNER->value)->where('page', PageEnum::ADD_SIGNAGE->value)->where('status', 'active')->latest()->get();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    return '<div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                                <a href="' . route('admin.tutorial.edit', ['id' => $data->id]) . '" type="button" class="btn btn-primary fs-14 text-white edit-icn" title="Edit">
        ...
        return view('backend.layouts.tutorial.index');
    }

    public function create()
    {
        return view('backend.layouts.tutorial.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
          ...
          if(Tutorial::create($validatedData)){
            return redirect()->route('admin.tutorial.index')->with('t-success', 'Created successfully');
          }
    ...
    public function edit(string $id)
    {
        ...
        return view("backend.layouts.tutorial.update", compact("homeAbout"));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            return redirect()->route('admin.tutorial.index')->with('t-success', 'Updated successfully');
        } catch (\Throwable $th) {
